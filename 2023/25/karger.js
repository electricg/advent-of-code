...
function kargerMinCut(graph) {
	...
	while (vertices > 2) {
		// let i = Math.floor(Math.random() * 10) % E; // this is a wrong translation
		const i = Math.floor(Math.random() * (E - 1));

		const subset1 = find(subsets, edge[i].src);
		const subset2 = find(subsets, edge[i].dest);
...
function main() {
	...
	const r = Math.random();
	const res = kargerMinCut(graph, r);
