...
const template = inputFormatted.map(({ min, max, password, letter }, i) => `
  <div class="password" data-val="${letter}" data-min="${min}" data-max="${max}" id="pwd${i + 1}">
    ${password.split('').map(
      char => `
        <span data-val="${char}">${char}</span>
      `
    ).join('')}
    <b></b>
    <i></i>
...
`).join('');
 ...
 document.getElementById('passwords').innerHTML = template;

// const xxx = inputFormatted.map(({ min, max, letter }, index) => `${min} ${max} ${letter}`).join(',');
